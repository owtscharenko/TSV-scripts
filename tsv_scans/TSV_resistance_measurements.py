...
#         marker = True
    ...
    workdir = '/media/niko/data/TSV-measurements/TSV-D7/resmeas'
    os.chdir(workdir)
    '''
    Scanning
    '''
    iv = IV('/home/niko/git/TSV-scripts/tsv_scans/devices.yaml')
#     if current > 0 :    
#         iv.scan_tsv_res_source_CURR(f, current, 1, 5000 , 0.0001, sensing, 'Sourcemeter1')
#     elif current < 0 :
    datafile =  iv.scan_tsv_res_VOLT(f, voltage, climit, 5000 , stepsize, 'Sourcemeter')                                                                  
    '''
    Plotting
    '''   
    p = (1,5,7);
